...
# TRAIL SETTINGS
# =========================
# Maximum number of points stored for the active trajectory trail.
MAX_TRAIL_POINTS = 40
...
# NEW: trail memory for the CURRENT active ball only.
# This should reset whenever the active ball switches.
active_trail = []
...
while True:
    ...
    if result.boxes is not None and len(result.boxes) > 0:
        for box in result.boxes:
            ...
            # Estimate candidate motion from nearest previous candidate
            cand["motion_est"] = estimate_motion_from_previous(cand, prev_candidates)

            # Mark whether candidate lies in the preferred launch area
    ...
    if not active_track["active"]:
        ...
        if best_init is not None:
            ...
            # NEW: start a brand-new trail for this newly initialized active ball.
    ...
    # ---------------------------------
        ...
        # Store previous active position so we know whether the active ball changed
        prev_active_cx = active_track["cx"]
        prev_active_cy = active_track["cy"]

        if match_ok:
            update_track_from_candidate(active_track, best_match, dt)

            # NEW: same active track continues -> extend the existing trail.
            append_to_trail(
                active_trail,
                active_track["cx"],
                active_track["cy"],
        ...
        if best_challenger is not None and best_challenger["in_launch_region"]:
            ...
            # NEW: when challenger is confirmed and takes over,
        ...
        if active_track["miss_count"] > MAX_MISSES:
            ...
            # NEW: if the track is lost completely, clear the trail.
            active_trail = []
    ...
    # ---------------------------------
    # NEW:
    # Draw line segments connecting the active ball's recent positions.
    # This visualizes the trajectory for the currently active ball only.
    ...
    # ---------------------------------
    # STEP 8: Highlight the ACTIVE ball
    # ---------------------------------
    ...
    # ---------------------------------
    # STEP 9: Highlight pending challenger
    # ---------------------------------
    ...
    # ---------------------------------
    # STEP 11: Save current candidates for next frame
    # ---------------------------------
